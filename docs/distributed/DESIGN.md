...
This document explains the design, architecture and advanced use cases of the Buckit distributed server.
...
Expansion of ellipses and choice of erasure sets based on this expansion is an automated process in Buckit. Here are some of the details of our underlying erasure coding behavior.

- Erasure coding used by Buckit is [Reed-Solomon](https://github.com/klauspost/reedsolomon) erasure coding scheme, which has a total shard maximum of 256 i.e 128 data and 128 parity. Buckit design goes beyond this limitation by doing some practical architecture choices.

- Erasure set is a single erasure coding unit within a Buckit deployment. An object is sharded within an erasure set. Erasure set size is automatically calculated based on the number of drives. Buckit supports unlimited number of drives but each erasure set can be up to 16 drives and a minimum of 2 drives.
...
- In this algorithm, we also make sure that we spread the drives out evenly. Buckit server expands ellipses passed as arguments. Here is a sample expansion to demonstrate the process.
...
- Buckit does erasure coding at the object level not at the volume level, unlike other object storage vendors. This allows applications to choose different storage class by setting `x-amz-storage-class=STANDARD/REDUCED_REDUNDANCY` for each object uploads so effectively utilizing the capacity of the cluster. Additionally these can also be enforced using IAM policies to make sure the client uploads with correct HTTP headers.

- Buckit also supports expansion of existing clusters in server pools. Each pool is a self contained entity with same SLA's (read/write quorum) for each object as original cluster. By using the existing namespace for lookup validation Buckit ensures conflicting objects are not created. When no such object exists then Buckit simply uses the least used pool to place new objects.
...
Refer to the sizing guide with details on the default parity count chosen for different erasure stripe sizes [here](https://buckit-io.github.io/docs/distributed/SIZING.md)

Buckit places new objects in server pools based on proportionate free space, per pool. Following pseudo code demonstrates this behavior.
