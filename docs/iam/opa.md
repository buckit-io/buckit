...
OPA is a lightweight general-purpose policy engine that can be co-located with Buckit server, in this document we talk about how to use OPA HTTP API to authorize requests. It can be used with any type of credentials (STS based like OpenID or LDAP, regular IAM users or service accounts).

OPA is enabled through Buckit's Access Management Plugin feature.
...
### 4. Setup Buckit with OPA

Set the `MINIO_POLICY_PLUGIN_URL` as the endpoint that Buckit should send authorization requests to. Then start the server.
