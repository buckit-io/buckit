...
WSO2 is an Identity Server open source and is released under Apache Software License Version 2.0, this document covers configuring WSO2 to be used as an identity provider for Buckit server STS API.
...
WSO2 generates tokens in first style by default, but if to be used with Buckit we should configure WSO2 to provide JWT tokens instead.
...
Using the above `id_token` we can perform an STS request to Buckit to get temporary credentials for Buckit API operations. Buckit STS API uses [JSON Web Key Set Endpoint](https://docs.wso2.com/display/IS541/JSON+Web+Key+Set+Endpoint) to validate if JWT is valid and is properly signed.
...
### 5. Setup Buckit with OpenID configuration URL

Buckit server expects environment variable for OpenID configuration url as `MINIO_IDENTITY_OPENID_CONFIG_URL`, this environment variable takes a single entry.
...
Assuming that Buckit server is configured to support STS API by following the doc [Buckit STS Quickstart Guide](https://buckit-io.github.io/docs/community/minio-object-store/developers/security-token-service.html), execute the following command to temporary credentials from Buckit server.
...
These credentials can now be used to perform Buckit API operations, these credentials automatically expire in 1hr. To understand more about credential expiry duration and client grants STS API read further [here](https://buckit-io.github.io/docs/sts/client-grants.md).
