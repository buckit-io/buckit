# How to secure access to Buckit on Kubernetes with TLS [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io)

This document explains how to configure Buckit server with TLS certificates on Kubernetes.

## 1. Prerequisites

- Familiarity with [Buckit deployment process on Kubernetes](https://buckit-io.github.io/docs/community/minio-object-store/operations/deployments/kubernetes.html).

- Kubernetes cluster with `kubectl` configured.

- Acquire TLS certificates, either from a CA or [create self-signed certificates](https://buckit-io.github.io/docs/community/minio-object-store/operations/network-encryption.html).
...
For testing purposes, here is [how to create self-signed certificates](https://buckit-io.github.io/docs/tls#3-generate-self-signed-certificates).
...
the Buckit server's config sub-directory that is used to store certificates. By default, the location is
...
so avoid setting `mountPath` to a path that Buckit server expects to write to.
