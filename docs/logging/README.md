# Buckit Logging Quickstart Guide [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io)

This document explains how to configure Buckit server to log to different logging targets.

## Log Targets

Buckit supports currently two target types
...
HTTP target logs to a generic HTTP endpoint in JSON format and is not enabled by default. To enable HTTP target logging you would have to update your Buckit server configuration using `mc admin config set` command.

Assuming `mc` is already [configured](https://buckit-io.github.io/docs/community/minio-object-store/reference/minio-mc.html#quickstart)
...
Buckit also honors environment variable for HTTP target logging as shown below, this setting will override the endpoint settings in the Buckit server config.
...
Buckit also honors environment variable for HTTP target Audit logging as shown below, this setting will override the endpoint settings in the Buckit server config.
...
```json
{
  ...
  "userAgent": "Buckit (linux; amd64) minio-go/v7.0.70 mc/RELEASE.2024-04-30T17-44-48Z",
  ...
  "requestHeader": {
    ...
    "User-Agent": "Buckit (linux; amd64) minio-go/v7.0.70 mc/RELEASE.2024-04-30T17-44-48Z",
...
Configure Buckit to send audit logs to locally running Kafka brokers
