# Buckit Server Config Guide [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io) [![Docker Pulls](https://img.shields.io/docker/pulls/minio/minio.svg?maxAge=604800)](https://hub.docker.com/r/minio/minio/)

## Configuration Directory

Buckit stores all its config as part of the server deployment, config is erasure coded on Buckit. On a fresh deployment Buckit automatically generates a new `config` and this config is available to be configured via `mc admin config` command. Buckit also encrypts all the config, IAM and policies content if KMS is configured. Please refer to how to encrypt your config and IAM credentials [here](https://buckit-io.github.io/docs/kms/IAM.md).

### Certificate Directory

TLS certificates by default are expected to be stored under ``${HOME}/.minio/certs`` directory. You need to place certificates here to enable `HTTPS` based access. Read more about [How to secure access to Buckit server with TLS](https://buckit-io.github.io/docs/community/minio-object-store/operations/network-encryption.html).

Following is a sample directory structure for Buckit server with TLS certificates.
...
On Buckit admin credentials or root credentials are only allowed to be changed using ENVs namely `MINIO_ROOT_USER` and `MINIO_ROOT_PASSWORD`.
...
By default, parity for objects with standard storage class is set to `N/2`, and parity for objects with reduced redundancy storage class objects is set to `2`. Read more about storage class support in Buckit server [here](https://buckit-io.github.io/docs/erasure/storage-class/README.md).
...
Buckit supports storing encrypted IAM assets in etcd, if KMS is configured. Please refer to how to encrypt your config and IAM credentials [here](https://buckit-io.github.io/docs/kms/IAM.md).

> NOTE: if *path_prefix* is set then Buckit will not federate your buckets, namespaced IAM assets are assumed as isolated tenants, only buckets are considered globally unique but performing a lookup with a *bucket* which belongs to a different tenant will fail unlike federated setups where Buckit would port-forward and route the request to relevant cluster accordingly. This is a special feature, federated deployments should not need to set *path_prefix*.
...
replication_max_lrg_workers     (number)    set the maximum number of replication workers Buckit uses to replicate large objects between sites. (default: '10')
...
Notification targets supported by Buckit are in the following list. To configure individual targets please refer to more detailed documentation [here](https://buckit-io.github.io/docs/community/minio-object-store/administration/monitoring.html#bucket-notifications).
...
* [Buckit Quickstart Guide](https://buckit-io.github.io/docs/community/minio-object-store/operations/deployments/baremetal-deploy-minio-on-redhat-linux.html)
* [Configure Buckit Server with TLS](https://buckit-io.github.io/docs/community/minio-object-store/operations/network-encryption.html)
