...
- Install `mc` - [mc Quickstart Guide](https://buckit-io.github.io/docs/community/minio-object-store/reference/minio-mc.html#quickstart)
...
This will only work with a versioned bucket, take a look at [Bucket Versioning Guide](https://buckit-io.github.io/docs/community/minio-object-store/administration/object-management/object-versioning.html) for more understanding.
...
This JSON rule is equivalent to the following Buckit Client command:
```
...
#### 3.2.a Automatic removal of noncurrent versions keeping only most recent ones immediately (Buckit only extension)

This is available only on Buckit as an extension to the NewerNoncurrentVersions feature. The following rule makes it possible to remove older noncurrent versions
...
#### 3.2.b Automatic removal of all versions (Buckit only extension)

This is available only on Buckit as an extension to the Expiration feature. The following rule makes it possible to remove all versions of an object under 
...
In Erasure mode, Buckit supports tiering to public cloud providers such as GCS, AWS and Azure as well as to other Buckit clusters via the ILM transition feature. This will allow transitioning of older objects to a different cluster or the public cloud by setting up transition rules in the bucket lifecycle configuration. This feature enables applications to optimize storage costs by moving less frequently accessed data to a cheaper storage without compromising accessibility of data.

To transition objects in a bucket to a destination bucket on a different cluster, applications need to specify a transition tier defined on Buckit instead of storage class while setting up the ILM lifecycle rule.
...
Note: In the case of S3, it is possible to create a tier from Buckit running in EC2 to S3 using AWS role attached to EC2 as credentials instead of accesskey/secretkey:
...
Note that transition event notification is a Buckit extension.

## Explore Further

- [Buckit | Golang Client API Reference](https://buckit-io.github.io/docs/community/minio-object-store/developers/go/API.html)
